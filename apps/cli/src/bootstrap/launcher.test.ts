...
function runLauncher(exitCodes: number[]) {
...
`,
  ...
  const result = spawnSync(process.execPath, [resolve('bin/dsh-console.js')], {
    env: {
      ...process.env,
      DSH_HOME: dshHome,
  ...
  return { result, count: Number(readFileSync(counter, 'utf8')) };
}
