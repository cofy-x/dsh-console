...
vi.mock('../shared/ansi-output.js', () => ({
  AnsiOutputText: function MockAnsiOutputText({ data }: { data: AnsiOutput }) {
    ...
    if (!data || !Array.isArray(data)) {
      return <Text>MockAnsiOutput:empty</Text>;
    }
...
describe('<ToolMessage />', () => {
  ...
  describe('JSON rendering', () => {
    it('pretty prints valid JSON', () => {
      ...
      const { lastFrame } = renderWithContext(
        <ToolMessage
          {...baseProps}
          resultDisplay={{ type: 'text', content: testJSONstring }}
    ...
    it('renders pretty JSON in ink frame', () => {
      const { lastFrame } = renderWithContext(
        <ToolMessage
          {...baseProps}
          resultDisplay={{ type: 'text', content: '{"a":1,"b":2}' }}
        />,
        StreamingState.Idle,
      );
    ...
    it('pretty printing 10kb JSON completes in <50ms', () => {
      ...
      const { lastFrame } = renderWithContext(
        <ToolMessage
          {...baseProps}
          resultDisplay={{ type: 'text', content: large }}
          renderOutputAsMarkdown={false}
        />,
        StreamingState.Idle,
      );
  ...
  it('renders DiffRenderer for diff results', () => {
    ...
    const { lastFrame } = renderWithContext(
      <ToolMessage
        {...baseProps}
        resultDisplay={{ type: 'diff', content: diffResult }}
      />,
      StreamingState.Idle,
    );
  ...
  it('renders AnsiOutputText for AnsiOutput results', () => {
    ...
    const { lastFrame } = renderWithContext(
      <ToolMessage
        {...baseProps}
        resultDisplay={{ type: 'ansi', content: ansiResult }}
      />,
      StreamingState.Idle,
    );
    expect(lastFrame()).toMatchSnapshot();
