...
vi.mock('../../../config/settings-schema.js', async (importOriginal) => {
  const original =
    await importOriginal<typeof import('../../../config/settings-schema.js')>();
  return {
    ...original,
...
vi.mock('../../contexts/vim-mode-context.js', async () => {
  const actual = await vi.importActual('../../contexts/vim-mode-context.js');
  return {
    ...actual,
    useVimMode: () => ({
      vimEnabled: false,
...
vi.mock('../../../config/settings-utils.js', async () => {
  const actual = await vi.importActual('../../../config/settings-utils.js');
  return {
    ...actual,
...
describe('SettingsDialog', () => {
  ...
  describe('Settings Navigation', () => {
    it.each([
      ...
      await waitFor(() => {
        expect(lastFrame()).toContain('Enable Prompt Completion');
      });
    ...
    it('wraps around when at the top of the list', async () => {
      ...
      await waitFor(() => {
        ...
        expect(lastFrame()).toContain('Use OSC 52 Paste');
      });
    ...
    it('should show restart prompt for restart-required settings', async () => {
      ...
      await waitFor(() => {
        expect(lastFrame()).not.toContain(
          'To see changes, DSH Console must be restarted',
        );
      });
    ...
    it.each([
      ...
      },
      {
        ...
        userSettings: {
          ...
          tools: {
            shell: { showColor: true, pager: 'less' },
          },
          context: {},
        },
        systemSettings: {},
