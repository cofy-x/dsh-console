...
export function SessionDialog({
  ...
  const resolveTitles = useCallback(
    (candidates: readonly SessionListItemView[]) => {
      const sessionsToResolve = candidates.filter(
        (session) =>
      ...
      void runtime
        ...
        .then((titles) => {
          ...
          setHighlighted((current) =>
            current ? withResolvedTitle(current) : current,
          );
          setPending((current) =>
            current ? withResolvedTitle(current) : current,
          );
  ...
  useEffect(() => {
    ...
    return () => {
      controller.abort();
      for (const titleController of titleRequests.values())
  ...
  const initialIndex = Math.max(
    0,
    sessions.findIndex((session) => session.current),
  );
  ...
  const resume = useCallback(
    async (session: SessionListItemView) => {
  ...
  const select = useCallback(
    (session: SessionListItemView) => {
      ...
      if (!session.resumable) {
        setError(
          session.resumeUnavailableReason ?? 'This Session cannot be resumed.',
        );
        return;
      }
    ...
    >
      <Box justifyContent="space-between">
        ...
        <DialogCloseAction
          onClose={dismiss}
          isActive={!switching}
          label={pending ? 'Esc to Sessions' : undefined}
        />
      </Box>
      {error && (
        <Box marginTop={1}>
      ...
      {loading ? (
        ...
        <Box marginTop={1}>
          <Text color={theme.text.secondary}>No resumable Sessions.</Text>
        </Box>
      ) : (
            ...
            {pending ? (
              <>
                ...
                <Box marginTop={1} flexDirection="column">
                  ...
                  <Text color={theme.text.secondary}>
                    The current transcript will be replaced.
                  </Text>
                </Box>
                <Box marginTop={1}>
                  <RadioButtonSelect
                    ...
                    onSelect={(confirmed) =>
                      confirmed ? void resume(pending) : setPending(undefined)
                    }
                    isFocused={!switching}
                    showNumbers={false}
                  />
              ...
              <>
                ...
                <Box marginTop={1} flexDirection="column">
                  ...
                  <Text>
                    {new Date(highlighted.createdAt).toLocaleString()}
                  </Text>
                  {!highlighted.resumable &&
                    highlighted.resumeUnavailableReason && (
                      <>
      ...
      <Box marginTop={1}>
        <Text color={theme.text.secondary}>
          Use ↑/↓ to navigate and Enter to resume.
        </Text>
      </Box>
    </Box>
  );
