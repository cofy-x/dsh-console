...
import type { UseShellHistoryReturn } from '../../hooks/input/use-shell-history.js';
import { useShellHistory } from '../../hooks/input/use-shell-history.js';
import type { UseCommandCompletionReturn } from '../../hooks/completion/use-command-completion.js';
import {
  CompletionMode,
  useCommandCompletion,
} from '../../hooks/completion/use-command-completion.js';
import type { UseInputHistoryReturn } from '../../hooks/input/use-input-history.js';
import { useInputHistory } from '../../hooks/input/use-input-history.js';
import type { UseReverseSearchCompletionReturn } from '../../hooks/completion/use-reverse-search-completion.js';
import { useReverseSearchCompletion } from '../../hooks/completion/use-reverse-search-completion.js';
...
import {
  ...
  clipboardHasImage,
  saveClipboardImage,
} from '../../../terminal/clipboard/index.js';
...
vi.mock('../../hooks/completion/use-command-completion.js');
vi.mock('../../hooks/input/use-input-history.js');
vi.mock('../../hooks/completion/use-reverse-search-completion.js');
vi.mock('clipboardy');
vi.mock('../../hooks/terminal/use-kitty-keyboard-protocol.js');
vi.mock('../../../terminal/clipboard/index.js');
...
describe('InputPrompt', () => {
  ...
  describe('clipboard image paste', () => {
    beforeEach(() => {
      ...
      vi.mocked(saveClipboardImage).mockResolvedValue(null);
      vi.mocked(cleanupOldClipboardImages).mockResolvedValue(undefined);
    });

    it('should handle Ctrl+V when clipboard has an image', async () => {
      vi.mocked(clipboardHasImage).mockResolvedValue(true);
      vi.mocked(saveClipboardImage).mockResolvedValue(
        '/test/.dsh-console-clipboard/clipboard-123.png',
      );
      ...
      await waitFor(() => {
        ...
        expect(saveClipboardImage).toHaveBeenCalledWith(
          props.config.getTargetDir(),
        );
        expect(cleanupOldClipboardImages).toHaveBeenCalledWith(
          props.config.getTargetDir(),
        );
    ...
    it('should handle image save failure gracefully', async () => {
      ...
      await waitFor(() => {
        expect(saveClipboardImage).toHaveBeenCalled();
      });
      expect(mockBuffer.setText).not.toHaveBeenCalled();
      unmount();
    });
    ...
    it('should insert image path at cursor position with proper spacing', async () => {
      const imagePath = path.join(
        'test',
        '.dsh-console-clipboard',
        'clipboard-456.png',
      );
      vi.mocked(clipboardHasImage).mockResolvedValue(true);
      vi.mocked(saveClipboardImage).mockResolvedValue(imagePath);
    ...
    it('should handle errors during clipboard operations', async () => {
      const debugLoggerErrorSpy = vi
        ...
        .mockImplementation(() => {});
      vi.mocked(clipboardHasImage).mockRejectedValue(
  ...
  it('should autocomplete supplied commands on Enter', async () => {
    const customCommand: SlashCommand = {
      name: 'find-capital',
      kind: CommandKind.DSH,
      description: 'Find capital of a country',
      action: vi.fn(),
      // No autoExecute flag, so the supplied command defaults to undefined.
    };
  ...
  it('should clear the screen when the clear-screen key is pressed', async () => {
    const { stdin, unmount } = renderWithProviders(<InputPrompt {...props} />, {
      uiActions,
    });
  ...
  describe('enhanced input UX - keyboard shortcuts', () => {
    ...
    it('does nothing on double ESC when the buffer is empty', async () => {
      const onEscapePromptChange = vi.fn();
      props.onEscapePromptChange = onEscapePromptChange;
      props.buffer.setText('');
