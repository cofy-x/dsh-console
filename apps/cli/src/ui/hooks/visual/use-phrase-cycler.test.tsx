...
} from '../visual/use-phrase-cycler.js';
import {
  INFORMATIVE_TIPS,
...
describe('usePhraseCycler', () => {
  ...
  it('should initialize with a witty phrase when not active', () => {
  ...
  it('should not cycle phrases if isActive is false', async () => {
    const { lastFrame } = render(
      <TestComponent isActive={false} />,
    );
  ...
  it('should cycle through phrases when isActive is true', async () => {
  ...
  it('should use custom phrases when provided', async () => {
    ...
    rerender(
      <TestComponent isActive={true} customPhrases={[]} />,
    );
    await act(async () => {
  ...
  it('should reset phrase when transitioning from inactive to active', async () => {
    vi.spyOn(Math, 'random').mockImplementation(() => 0.5); // Always witty for subsequent phrases
    const { lastFrame, rerender } = render(
      <TestComponent isActive={true} />,
    );
    ...
    // Deactivating resets to the first configured loading phrase.
    rerender(<TestComponent isActive={false} />);
    await act(async () => {
      await vi.advanceTimersByTimeAsync(0);
    });
    expect(lastFrame()).toBe(WITTY_LOADING_PHRASES[0]);
