...
import { MAX_SUGGESTIONS_TO_SHOW } from '../../components/input/suggestions-display.js';
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        const searcher = FileSearchFactory.create({
          ...
          useDshConsoleIgnore:
            config?.getFileFilteringOptions()?.respectDshConsoleIgnore ?? true,
          cache: true,
    ...
    const search = async () => {
      ...
      try {
        ...
        dispatch({ type: 'SEARCH_SUCCESS', payload: fileSuggestions });
      } catch (error) {
