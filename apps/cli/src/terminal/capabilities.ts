...
export class TerminalCapabilityManager {
  ...
  private cleanupOnExit: (() => void) | undefined;
  ...
  private registerExitCleanup(): void {
    ...
    this.cleanupOnExit = () => {
      // Don't bother catching errors since if one write fails, the other
      // cleanup writes probably will too.
      disableKittyKeyboardProtocol();
      disableModifyOtherKeys();
      disableBracketedPasteMode();
    };
