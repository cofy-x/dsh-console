...
// --- Configuration ---
const cliPackageDir = path.resolve('apps', 'cli'); // Base directory for the CLI package
...
if (!buildMtime) {
  ...
  const errorMessage = `ERROR: Build timestamp file (${path.relative(process.cwd(), buildTimestampPath)}) not found. Run \`pnpm run build\` first.`;
...
if (newerSourceFileFound) {
  const finalWarning =
    '\nRun "pnpm run build" to incorporate changes before starting.';
