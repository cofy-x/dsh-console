...
const generatedCliDir = join(root, 'apps/cli/src/generated');
const cliGitCommitFile = join(generatedCliDir, 'git-commit.ts');
const cliPackageFile = join(root, 'apps/cli/package.json');
...
try {
  ...
  const packageJson = JSON.parse(readFileSync(cliPackageFile, 'utf8'));
...
const fileContent = `/**
 * @license
 * Copyright ${new Date().getUTCFullYear()} cofy-x
 * SPDX-License-Identifier: Apache-2.0
 */
