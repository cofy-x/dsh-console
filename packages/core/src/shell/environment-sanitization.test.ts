...
import {
  ...
  sanitizeEnvironment,
} from './environment-sanitization.js';
...
describe('sanitizeEnvironment', () => {
  ...
  it('should allow variables prefixed with DSH_CONSOLE_', () => {
    const env = {
      DSH_CONSOLE_FOO: 'bar',
      DSH_CONSOLE_BAZ: 'qux',
    };
  ...
  it('should redact variables with values matching all token and credential patterns', () => {
    const stripeToken = (
      kind: 'sk' | 'rk',
      mode: 'live' | 'test',
    ): string => [kind, mode, 'x'.repeat(24)].join('_');
    const env = {
      // GitHub
      ...
      // Stripe
      STRIPE_SK_LIVE: stripeToken('sk', 'live'),
      STRIPE_RK_LIVE: stripeToken('rk', 'live'),
      STRIPE_SK_TEST: stripeToken('sk', 'test'),
      STRIPE_RK_TEST: stripeToken('rk', 'test'),
      // Slack
