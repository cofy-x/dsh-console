...
vi.mock('../shell/pty.js', () => ({
  getPty: mockGetPty,
}));
vi.mock('../shell/terminal-serializer.js', () => ({
...
describe('ShellExecutionService', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    // Ensure UTF-8 encoding is used for multi-byte character tests
    // This is critical when running tests in VS Code where LANG may not be set
...
describe('ShellExecutionService child_process fallback', () => {
  ...
  describe('Successful Execution', () => {
    ...
    it.todo(
      'should truncate stdout using a sliding window and show a warning',
      async () => {
        ...
        const truncationMessage =
          '[DSH_CONSOLE_WARNING: Output truncated. The buffer is limited to 16MB.]';
...
describe('ShellExecutionService environment variables', () => {
  ...
  it('should use a sanitized environment when in a GitHub run', async () => {
    ...
    vi.stubEnv('DSH_CONSOLE_TEST_VAR', 'test-value');
    ...
    expect(ptyEnv).toHaveProperty('DSH_CONSOLE_TEST_VAR', 'test-value');
    ...
    expect(cpEnv).toHaveProperty('DSH_CONSOLE_TEST_VAR', 'test-value');
  ...
  it('should include the full process.env when not in a GitHub run', async () => {
    ...
    expect(ptyEnv).toHaveProperty('DSH_CONSOLE', '1');
    ...
    expect(cpEnv).toHaveProperty('DSH_CONSOLE', '1');
