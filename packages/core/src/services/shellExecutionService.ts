...
import type { PtyImplementation } from '../shell/pty.js';
import { getPty } from '../shell/pty.js';
import { spawn as cpSpawn } from 'node:child_process';
import { TextDecoder } from 'node:util';
import os from 'node:os';
import type { IPty } from '@lydell/node-pty';
import { getCachedEncodingForBuffer } from '../fs/encoding.js';
import {
...
import type { EnvironmentSanitizationConfig } from '../shell/environment-sanitization.js';
      ...
      const result = new Promise<ShellExecutionResult>((resolve) => {
        ...
        const handleExit = (
          ...
          if (stdoutTruncated || stderrTruncated) {
            const truncationMessage = `\n[DSH_CONSOLE_WARNING: Output truncated. The buffer is limited to ${
    ...
    try {
      ...
      const result = new Promise<ShellExecutionResult>((resolve) => {
        ...
        ptyProcess.onExit(
          ({ exitCode, signal }: { exitCode: number; signal?: number }) => {
            ...
            void Promise.race([processingComplete, abortFired]).then(() => {
              finalize();
            });
          },
        );
      ...
      if (error.message.includes('posix_spawnp failed')) {
        onOutputEvent({
          type: 'data',
          chunk:
            '[DSH_CONSOLE_WARNING] PTY execution failed, falling back to child_process. This may be due to sandbox restrictions.\n',
        });
        throw e;
      } else {
        return {
