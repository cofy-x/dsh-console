...
export function loadIgnoreRules(options: LoadIgnoreRulesOptions): Ignore {
  ...
  if (options.useDshConsoleIgnore) {
    const dshConsoleIgnorePath = path.join(
      options.projectRoot,
      '.dsh-consoleignore',
    );
    if (fs.existsSync(dshConsoleIgnorePath)) {
      ignorer.add(fs.readFileSync(dshConsoleIgnorePath, 'utf8'));
    }
  }
