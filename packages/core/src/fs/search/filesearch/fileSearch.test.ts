...
import { createTmpDir, cleanupTmpDir } from '@cofy-x/dsh-console-test-utils';
...
describe('FileSearch', () => {
  ...
  it('should use .dsh-consoleignore rules', async () => {
    tmpDir = await createTmpDir({
      '.dsh-consoleignore': 'dist/',
      dist: ['ignored.js'],
      src: ['not-ignored.js'],
    });
  ...
  it('should combine .gitignore and .dsh-consoleignore rules', async () => {
    tmpDir = await createTmpDir({
      '.gitignore': 'dist/',
      '.dsh-consoleignore': 'build/',
      dist: ['ignored-by-git.js'],
      build: ['ignored-by-cog.js'],
      src: ['not-ignored.js'],
    });
  ...
  it('should not use fzf for fuzzy matching when enableFuzzySearch is true', async () => {
    tmpDir = await createTmpDir({
      src: {
        'file1.js': '',
        'flexible.js': '',
  ...
  it('should use fzf for fuzzy matching when enableFuzzySearch is false', async () => {
    tmpDir = await createTmpDir({
      src: {
        'file1.js': '',
        'flexible.js': '',
