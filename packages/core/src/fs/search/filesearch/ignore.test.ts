...
describe('loadIgnoreRules', () => {
  ...
  it('should load rules from .dsh-consoleignore', async () => {
    tmpDir = await createTmpDir({
      '.dsh-consoleignore': '*.log',
    });
    const ignore = loadIgnoreRules({
      projectRoot: tmpDir,
      useGitignore: false,
  ...
  it('should combine rules from .gitignore and .dsh-consoleignore', async () => {
    tmpDir = await createTmpDir({
      '.gitignore': '*.log',
      '.dsh-consoleignore': '*.txt',
    });
    const ignore = loadIgnoreRules({
      projectRoot: tmpDir,
      useGitignore: true,
